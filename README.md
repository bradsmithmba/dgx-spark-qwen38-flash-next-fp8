...
One thing I found along the way that is also worth pointing out... on the newer DGX OS build, the CX7 disappears from the PCI bus entirely when no cable is inserted. A hot-plug driver keyed on cable presence (visible in a sysfs `debug_state` flag, 0 or 1) removes the device rather than just marking the link down. The first time I saw this, it looked exactly like dead hardware. It is not. It is a driver doing what it was told.
...
6. **FP8 KV loads fine and then fails on the first decode.** `--kv-cache-dtype fp8_e4m3` allocated the KV pool without complaint (209,472 tokens, 1.3 GB), but the first decode through the SM121 sparse-attention kernel raised `ValueError: unsupported SM121 QSA call: expected BF16 D=256, 12:1 GQA, TP1 24Q/2KV or TP2 12Q/1KV, bs<=128, and selected KV<=2055`. The checkpoint also ships no KV scale factors at all (`Using FP8 KV cache but no scaling factors provided. Defaulting to scaling factors of 1.0`), which would have been its own problem. Fix: `--kv-cache-dtype auto`, meaning BF16. The cost turned out smaller than I expected, about 1 GB, because only 12 of the model's 48 layers carry attention KV at all (the other 36 are Gated-DeltaNet, running a fixed recurrent state instead). The BF16 pool held 124,480 tokens at first (0.71 GB K plus 0.71 GB V), settling to 114,688 after the systemd cutover described below. Update, 2026-09-05: this wall fell. See below.

**Update, 2026-09-05: wall six fell.** A forum reply from user jahnclawdmonet on the NVIDIA thread linked above pointed at upstream sglang PR #36644, "[Qwen3.8] Fix FP8 KV cache support in QSA" (open, unreviewed at the time, stacked on PR #36497, already validated upstream on RTX 5090 and RTX PRO 6000 with the NVFP4 checkpoint). The patch is 879 lines of Python, touching `qsa/kernel.py`, `qsa/sparse_attn.py`, `qwen_sparse_attn_backend.py`, and a test file, and it applied cleanly with `git apply` against this image's sglang commit (`g593134d17`). The mechanism: the SM121 QSA kernel dequantizes the FP8 KV scratch back to the query dtype before the attention call, using a per-layer descale factor, rather than requiring the kernel itself to consume FP8 natively. Since the checkpoint ships no KV scale factors, the descale defaults to 1.0, and the server logs "Using FP8 KV cache but no scaling factors provided. Defaulting to scaling factors of 1.0" at startup, same message as before, now informational rather than a precursor to a crash.

At 65,536 context with the patch applied, the KV pool holds 178,624 tokens (0.51 GB K plus 0.51 GB V) against 114,688 to 124,480 under BF16 (0.71 GB K plus 0.71 GB V). Benchmarked with thinking off and 512-token completions: time to first token 0.17 to 0.18 s, single-stream 31.3 to 36.6 tok/s, 4-stream aggregate 90.3 to 91.0 tok/s. Needle-in-context recall was correct at roughly 4.6K, 20.6K, and 52.2K prompt tokens, with no token-id-0 corruption in any test.

The configuration was then promoted to 131,072 context, which is what `launch.sh` in this repository now runs in production. The KV pool at that context holds 242,944 tokens (0.70 GB K plus 0.70 GB V), with 9.98 GB of available GPU memory remaining on the head node after CUDA graph capture, weight load taking 483.78 s, and the MTP draft head taking 52.53 s to load. Needle recall was independently verified correct at 16,817, 101,066, and 125,596 prompt tokens. Throughput held steady against the BF16 65K baseline above: single-stream 35.7 to 37.0 tok/s, 2-stream aggregate 59.4 tok/s, 4-stream aggregate 92.8 tok/s, time to first token 0.15 to 0.19 s. One benchmarking pitfall worth recording: running two clients against the same endpoint at once roughly halves the measured per-client and aggregate throughput, so every number above was measured with a single benchmark client running alone.

The pre-patch image was retagged (`sglang-spark:fp8-bf16kv`) and the pre-patch `launch.sh` kept as a backup before rebuilding, so reverting is a retag plus a service restart rather than a rebuild. See `pr36644.diff` in this repository for the patch itself, applied by `Dockerfile` at build time.
...
The table above is the original BF16 KV, 65,536-context baseline. After the FP8 KV cache patch (PR #36644, see the wall-six update above) and promotion to 131,072 context on 2026-09-05, the measured numbers are:

| Metric | Result |
|---|---|
| Single-stream throughput | 35.7 to 37.0 tok/s |
| Time to first token | 0.15 to 0.19 s |
| 2 concurrent streams, aggregate | 59.4 tok/s |
| 4 concurrent streams, aggregate | 92.8 tok/s |
| KV pool | 242,944 tokens (0.70 GB K plus 0.70 GB V) |
| Available GPU memory after CUDA graph capture | 9.98 GB |

Performance is neutral against the BF16 65K baseline above, at double the context length and roughly a third of the KV pool's memory footprint.
...
The FP8 KV question is answered: it works, with PR #36644 applied, at both 65K and 131K context, credit to jahnclawdmonet's forum reply for the pointer. What I'm still turning over now is calibrated KV scales: the checkpoint ships none, so every run above uses the patch's default descale of 1.0 rather than a value derived from the model's own activations, and I have not measured what a calibrated scale would change. The other open question is whether the SM121 FlashInfer decode path (upstream PRs #36649 and #36806) can be made safe above 120K context, since that path is a separate piece of the puzzle from the QSA kernel this patch touches.
...
```
Each box builds its own local image derivative from the public `lmsysorg/sglang:qwen38flashnext` base. Image ids will differ between boxes because of build timestamps. As of 2026-09-05, the build also applies `pr36644.diff` (in this repository) for FP8 KV cache support: see the wall-six update above.

### Launch
`launch.sh` drops the page cache, removes any existing `sglang_node` container, and runs `python3 -m sglang.launch_server` with `--tp-size 2 --ep-size 2 --nnodes 2 --node-rank N --dist-init-addr $HEAD_IP:29511` over the ConnectX-7 RoCE link, speculative decoding (NEXTN, 3 steps, topk 1, 4 draft tokens), fp8 quantization, FP8 KV cache (`--kv-cache-dtype fp8_e4m3`, PR #36644 applied at build time, see the wall-six update above), and `--context-length 131072`. `MODEL_DIR` and `HF_CACHE` are environment variables with sane defaults. Override them for a different layout if needed. `HEAD_IP` has no default: set it to the head node's address on the cluster's fast interconnect. Order matters at startup: start rank 0 (the rendezvous master) within about 30 seconds before or after rank 1.
